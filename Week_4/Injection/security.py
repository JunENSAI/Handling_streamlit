...
def show_main_app():
    ...
    with st.sidebar:
        ...
        with st.expander("Leave a System Note"):
            st.caption("Try typing: <script>alert('Hacked')</script>")
            
            user_note = st.text_area("Enter note")
            
            if st.button("Post Note"):
                st.write("---")
                
                # BAD WAY (Demonstration only - Don't use in prod)
                #st.markdown(f"Raw: {user_note}", unsafe_allow_html=True) 
                
                # safe way
                safe_note = html.escape(user_note)
                st.markdown(f"Escaped: {safe_note}", unsafe_allow_html=True)
